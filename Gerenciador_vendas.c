...
int escolha_produto, altera_opcao, dia_alterar, dia_exibir,produto_exibir,  mais_vendido[1][1], menos_vendido[1][1],posicao_nome_mais, posicao_nome_menos,posicao_venda_mais, posicao_venda_menos;
...
void calculo_exibe_dados_produto(){
    ...
    for(i = 0; i<10; i++){
        for(j = 0; j<7; j++){
            ...
            if (i==0 && j==0){
            mais_vendido[1][1] = total_venda_produto_unidade[0];
            menos_vendido[1][1]=total_venda_produto_unidade[0];
            }
...
void gera_relatorio(FILE *arq1){
    ...
    fprintf(arq1,"\nTOTAL DAS VENDAS (EM REAIS): %.2f",total_venda_de_todos_valor);
     ...
     //MAIS E MENOS VENDIDOS
    fprintf(arq1,"\nProduto que mais vendeu (R$):  %f", mais_vendidoRS[1]);
    fprintf(arq1,"\nProduto que menos vendeu (R$):  %f", menos_vendidoRS[1]);

    //MAIS E MENOS VENDIDOS UNIDADE
    fprintf(arq1,"\nProduto mais vendeu (UN):  %i", mais_vendido[1]);
    fprintf(arq1,"\nProduto que menos vendeu (UN):  %i", menos_vendido[1]);
}
...
arq1 = fopen("Relatório (saida).txt", "wt");  // GRAVAÇÃO
